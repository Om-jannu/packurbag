...
class _TodoListScreenState extends State<TodoListScreen> {
  ...
  Future<void> fetchTodos() async {
    ...
    final response = await http.get(
      Uri.parse('http://${widget.serverIp}:5000/todos/$userId'),
    );

    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      final List<dynamic> todosData = data['data'];

      setState(() {
        todos = todosData.map((todo) => TodoItem.fromJson(todo)).toList();
      });
    } else {
      // Handle error
      print('Failed to fetch todos');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ? Center(child: CircularProgressIndicator())
          : ListView.builder(
              itemCount: showTodayOnly ? 1 : todos.length,
              itemBuilder: (context, index) {
                if (showTodayOnly) {
                  // Filter todos for today
                  DateTime now = DateTime.now();
                  String formattedToday = DateFormat('MMMM d, y').format(now);

                  List<TodoItem> todayTodos = todos.where((todo) {
                    String formattedDate =
                        DateFormat('MMMM d, y').format(todo.date);
...
class TodoItem {
  ...
  TodoItem({required this.text, required this.date, this.isChecked = false});

  factory TodoItem.fromJson(Map<String, dynamic> json) {
    return TodoItem(
      text: json['text'],
      date: DateTime.parse(json['date']),
      isChecked: json['completed'] ?? false,
    );
  }
